...
- `ci_feedback.max_log_lines`: integer, default `50`; log tail lines from failing checks (`0` disables)
- `ci_feedback.escalation`: string, default `label`; action on retry exhaustion (`label` or `comment`)
- `ci_feedback.escalation_label`: string, default `needs-human`; label applied during `label` escalation
